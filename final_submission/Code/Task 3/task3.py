    ...
    for epoch in range(epochs):
        ...
        if epoch == 0:
            if STRATEGY == "lora" or STRATEGY == "dora":
                ...
                save_dir = f"trained_models/xglm_{STRATEGY}_new_{load_split_size}_{datetime.now().strftime('%Y-%m-%d_%H-%M')}"
...
if __name__ == "__main__":
    ...
    # save the model to full_pt
    os.makedirs("trained_models", exist_ok=True)

    # if not os.path.exists(f"trained_models/full_model.pth"):
    #     torch.save(model.state_dict(), f"trained_models/full_model.pth")
    ...
    if STRATEGY == "lora":
        ...
        # model = AutoModelwithiA3(model)
        # model = iA3Adaptation(model)

        os.makedirs("trained_models/iA3", exist_ok=True)
        if not os.path.exists(f"trained_models/iA3/full_model.pt"):
            torch.save(model.state_dict(), f"trained_models/iA3/full_model.pt")
        modify_model_for_iA3(model)
        # load weights from the full model
        model.load_state_dict(torch.load(f"trained_models/iA3/full_model.pt"), strict=False)
