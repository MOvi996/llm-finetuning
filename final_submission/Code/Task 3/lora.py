...
class AutoModelwithLoRA(torch.nn.Module):
    def __init__(self, model_name, rank, alpha, output_hidden_states=False):
        super().__init__()
        self.model = AutoModelForCausalLM.from_pretrained(model_name, output_hidden_states=output_hidden_states)
        self.rank = rank
        self.alpha = alpha
